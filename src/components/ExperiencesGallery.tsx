...
export default function ExperiencesGallery({ locale }: ExperiencesGalleryProps) {
  const copy = siteTranslations[locale].experiences;

  return (
    <section
      id="experiences"
      className="relative overflow-hidden bg-brand py-20 sm:py-24 lg:py-32"
    >
      <div
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 lg:gap-6">
          {galleryImages.map((image) => (
            <GalleryCard key={image.id} image={image} locale={locale} />
          ))}
        </div>
